...
def opros(proc_rasch=1, lab_density=1):
    try:
        ...
        try:
            ...
            fast_update.fastupdate(cursor, conf_dict, d_c)
            cnn_mydb.commit()
...
#        if not HFlag :
            ...
            try:
                ...
                open(os.curdir + '\\report_error.txt', 'a').write(
                    'Error on archiv created : ' + str(Er) + ': ' + datetime.today().strftime(
                        "%d.%m.%Y %H:%M:%S") + '\n')
            ...
            try:
                ...
                for st in cursor.fetchall():
                    for i in range(1, 32):
                        ...
                        if 'P_'+ str(i) in d_c.keys():
                            cursor.execute("UPDATE %s SET P_" % (conf_dict['Table_work']) + str(
                                            i) + "= %s WHERE Shifr='%s'" % (0, st[d_c['Shifr']]))
