...
def opros_shifra(conf_dict, Shifr='', db='', zn=0):
    try:
        cnn_rem = sqlite3.connect(os.curdir + conf_dict['DATABASE_REM'])
        curs = cnn_rem.cursor()
...
def fastupdate(cursor, conf_dict, d_c):
    try:
        ...
        for st in cursor.fetchall():
            ...
            result=Gt=Gt_temp=list_zn0=list_zn1=list_zn2=result=Sz_hour=Sz_hour_m=Density=0
            list_zn=[]
            for sh in str(conf_dict['shifrs']).split(','):
                Zn = 'Zn_' + sh.split('_')[1]
                Gt_temp = opros_shifra(conf_dict, st[d_c[sh]], conf_dict['Table_rem'], st[d_c[Zn]])
                if Gt_temp:
            ...
            if -1 in list_zn:
                ...
                # list_zn2 = list_zn[2] if list_zn[2]>=0 else 0

                # if st[d_c['Shifr']] == 'R2':
                #     result = list_zn0 + list_zn1 + list_zn2
                # elif st[d_c['Shifr']] in ('R1','FI1001'):
                #     result = list_zn0 + list_zn1
                # else:
                result = list_zn[0]
            else:
                result = -99999
            ...
            #         Sz_hour = st[d_c['Sz_hour_m']] + (
            #             result * st[d_c['P_nom']] / 1000 - st[d_c['Sz_hour_m']]) / (st[d_c['n']] + 1)

            #     elif st[d_c['Prizn']] == 'NEFT':

            #         Sz_hour = st[d_c['Sz_hour_m']] + (result * (
            #             st[d_c['P_nom']] / 1000 - (0.001828 - 0.00132 * st[d_c['P_nom']] / 1000) * (
            #             list_zn[3] - 20))- st[d_c['Sz_hour_m']]) / (st[d_c['n']] + 1)
